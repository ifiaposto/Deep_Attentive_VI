...
#     https://github.com/tensorflow/probability/blob/v0.13.0/tensorflow_probability/python/layers/weight_norm.py
#
# All Rights Reserved.
...
def weightnorm_layer(layer,use_weight_norm,use_data_init):
        """
...
class WeightNorm(tf.keras.layers.Wrapper):
  ...
  def _compute_weights(self):
    ...
    self.layer.kernel = tf.nn.l2_normalize(
        self.v, axis=self.kernel_norm_axes) * tf.expand_dims(tf.math.exp(self.log_g), new_axis)
    #tf.keras.backend.print_tensor(tf.reduce_mean(self.v),message='unormalized normalized weight')
    #tf.keras.backend.print_tensor(tf.reduce_mean(self.layer.kernel),message='normalized weight')

  def _init_norm(self):
    """Set the norm of the weight vector."""
    kernel_norm = tf.sqrt(
        tf.reduce_sum(tf.square(self.v), axis=self.kernel_norm_axes))
    self.log_g.assign(tf.math.log(kernel_norm)+1e-2)
  ...
  def _data_dep_init(self, inputs):
    ...
    log_scale_init = -tf.math.log( tf.sqrt(v_init )+ 1e-5)
    self.log_g.assign(log_scale_init)
  ...
  def build(self, input_shape=None):
    ...
    if not self.layer.built:
      ...
      self.log_g = self.add_weight(
          name='g',
          shape=(int(self.v.shape[self.filter_axis]),),
          initializer=init_ops_v2.Constant(tf.math.log(kernel_norm+1e-2).numpy()),
